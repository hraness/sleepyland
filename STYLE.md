...
- The guides are health-adjacent. Show the health limit from `app/research/editorial-disclosure.ts` once on every guide and on the research index.
- Sleepyland pages carry no AI-drafting labels and no review claims. `docs/editorial-method.md` records who drafts the guides and which reviews have happened.
- Read the paper at each link before you cite it. After you add or change a PubMed, PMC, or DOI source, run `bun run check:citations --write`: the registry's title, journal, and year must match the linked record, and a preprint must be labeled as one.
- Research titles and `seoDescription` values are tuned to measured search queries. When you change one, name the query data behind the change in the pull request.
